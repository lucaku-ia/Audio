...
| Home, Player, Instrumentation dashboard | Not started on `main` — Home and the Instrumentation dashboard are built and in open PRs, see below; Player has no code anywhere yet | — | — |
...
(Pending-version promotion, Search keyword search, and Notifications/Settings — this
branch's own contents — are reflected directly in the status table above, not listed
here as a separate open PR.)
...
migration- or SQL-risky). PRs #1 and #2 (and this branch) were additionally reviewed
by a separate adversarial pass focused on security and cross-customer data isolation —
findings from those reviews are fixed in the branches themselves, not left as
follow-up items, except where explicitly noted as a documented, lower-priority gap.
#4 and #5 have not had that extra pass yet (session time constraints) — worth one
before merging.
