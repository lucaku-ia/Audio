...
/// otherwise an even split across lines within the simulated block progress
/// (see PlayerViewModel's class doc on the UI-only transport).
    ...
    private var lines: [TranscriptLine] {
        ...
        if let wordTimestamps = block.wordTimestamps, !wordTimestamps.isEmpty {
            return TranscriptLine.realLines(
                ...
                currentTime: viewModel.absolutePlaybackTimeS
            )
        }
