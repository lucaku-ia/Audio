"""Profile — System Contracts v0.1. Derivado, nunca preguntado ni mostrado como etiqueta."""
import enum
import uuid
...
class Profile(Base):
    ...
    delivery_time: Mapped[time | None] = mapped_column(Time, nullable=True)  # local time; tz va en delivery_timezone
    delivery_timezone: Mapped[str] = mapped_column(String(50), default="America/Bogota")
    max_length_minutes: Mapped[int | None] = mapped_column(Integer, nullable=True)  # null = sin límite, es techo no meta
